...
# Suppress the NotOpenSSLWarning from urllib3
...
def categorize_exoplanets():
    df = pd.read_csv("nasa_exoplanets.csv", low_memory=False)
    
    # Example criteria for categorization (simplified)
    habitable_zone = df[(df['pl_orbper'] > 200) & (df['pl_orbper'] < 400)]
    
    # Define 'habitable' column based on example criteria
    df['habitable'] = ((df['pl_orbper'] > 200) & (df['pl_orbper'] < 400)).astype(int)
    
    habitable_zone.to_csv("habitable_exoplanets.csv", index=False)
    print("Categorized exoplanets and saved to habitable_exoplanets.csv")

def train_ml_model():
    df = pd.read_csv("habitable_exoplanets.csv", low_memory=False)
    features = df[['pl_orbper', 'pl_rade', 'pl_bmasse']].fillna(0)  # Example features
